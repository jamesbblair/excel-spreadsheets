...
public class Move {
    ...
    public static int map(int y, int x) {
        int inv = 7 - y;
    ...
    public Move(LinkedList<Integer> places) {
        from = places.getFirst();
        to = places.getLast();
        jumps = new LinkedList<Jump>();
        ...
        if (hasJumps(from, to))
            for (Integer i : places)
                if (prev < 0)
                    ...
                    addJump(new Jump(prev, i, over(prev, prev = i)));
    }
    ...
    private boolean hasJumps(int f, int t) {
        int d = Math.abs(f - t);
        return d != 4 && d != 5;
    }
    ...
    public static void main(String[] args) {
        ...
        ll.add(21);
        ll.add(29);
        System.out.println(new Move(ll));
    }
}
